...
potential_killers <- inner_join(person, suspect_membership, by = c('id' = 'person_id'))
...
suspect_drivers_license <- inner_join(jan9_suspect_at_gym, drivers_license, by = c('license_id' = 'id')) #we can also do this a lot better in class I was just nosey

## look at 20180115 in facebook events
event_info <- left_join(day_of_crime, facebook_event_checkin, by = c('date' = 'date'))

suspect_facebook_event <- inner_join(suspect_drivers_license, event_info, by = c('id' = 'person_id'))
...
killer_description <- drivers_license %>% 
  filter(height >= 65, height <= 67,
         hair_color == 'red',
         car_make == "Tesla",
         car_model == "Model S")
killer_person <- inner_join(killer_description, person, by = c('id' = 'license_id'))

SQL_Symphony <- facebook_event_checkin %>% filter(str_starts(event_name, 'SQL Symphony'), str_starts(date, "201712")) %>% count(person_id) %>% filter(n == '3')

killer_SQL_Event <- inner_join(killer_person, SQL_Symphony, by = c('id.y' = 'person_id'))
income_killer <- inner_join(killer_SQL_Event, income) #310,000 annually!!!!!!

miranda_interview <- interview %>% filter(person_id == '99716')
miranda_interview$transcript #no transcript! 
#Miranda Priestly#
